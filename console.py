...
def cycle():
    active = True
    sempfiross.load_users()
    print("Welcome to sempfiross console\nEnter command:")
    while active:
        command = input(">")
            ...
            if len(command) == 2:
                ...
                if len(command[1]) == 2:
                    if command[1][0] == "midnight":
                        try:
                            userinfo = sempfiross.db.get_from_users(command[1][1])
                            log(False, "Midnight flag: "+str(userinfo["midnight"]))
                            sempfiross.bot[int(command[1][1])].event("midnight")
                        ...
                        try:
                            ...
                            if userinfo["midnight"]:
                                sempfiross.bot[int(command[1][1])].change_flag('midnight', False)
                            else:
                                sempfiross.bot[int(command[1][1])].change_flag('midnight', True)
                            userinfo = sempfiross.db.get_from_users(command[1][1])
                            log(False, "Midnight flag: "+str(userinfo["midnight"]))
                        except KeyError as e:
                            log(True, "Bot object is not exist: "+str(e))
                        ...
                        try:
                            sempfiross.bot[user_id] = sempfiross.VkBot(command[1][1])
                            log(False, "Bot-object has been created")
                        except Exception as e:
                            log(True, "Can't create bot-object - "+str(e))
                    elif command[1][0] == "delete":
                        try:
                            del sempfiross.bot[int(command[1][1])]
                            sempfiross.db.delete_user(command[1][1])
                ...
                if sempfiross.bot:
                    log(False, f"There is {len(sempfiross.bot)} bot-objects")
                    for i in sempfiross.bot:
                        print(sempfiross.bot[i])
                else:
